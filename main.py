...
class SqlServer():
    def __init__(self, server, database,user, password) -> None:
        ...
        # Fallback automático de drivers
        driver_candidates = []
        ...
        for driver in driver_candidates:
            try:
                ...
                pyodbc.connect(conn_str_test, timeout=3).close()
                self._connectionString = conn_str_test
                break
            except Exception:
                continue
        else:
            raise RuntimeError("Nenhum driver ODBC compatível foi encontrado para conectar ao SQL Server")
